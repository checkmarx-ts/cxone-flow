...
class ProjectNamingService:
    ...
    async def __call_timer(self):
        ...
        try:
            ...
            ProjectNamingService.log().info(
                f"{self.__naming_coro.__module__}.{self.__naming_coro.__name__} script execution time: {perf_counter_ns() - start}ns"
            )
    ...
    async def get_project_name(self, default_name: str, context: EventContext) -> str:
        ...
        async with self.__call_timer():
            try:
                ...
                if name is None:
                    ProjectNamingService.log().warning(
                        f"Naming module [{self.__naming_coro.__name__}] returned None, using default name {default_name}."
                    )
                return name if name is not None else default_name
            except BaseException as ex:
                ProjectNamingService.log().info(
                    f"Exception thrown by naming module [{self.__naming_coro.__name__}].  Using default name [{default_name}]."
                )
