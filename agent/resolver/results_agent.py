...
class ResolverResultsAgent(CxOneFlowAbstractWorkflowService):
    ...
    @staticmethod
    def __orchestrator_factory(
        orchestrator_name: str, context: EventContext
    ...
    async def __call__(self, msg: aio_pika.abc.AbstractIncomingMessage):
        ...
        try:
            if not self.__services.resolver.signature_valid(
                result_msg.details_signature, result_msg.details.to_binary()
            ):
                ResolverResultsAgent.log().error(
                    f"Message signature is invalid, scan not processed for project {result_msg.details.project_name}"
                ...
                if result_msg.state == ScanStates.FAILURE:
                    if result_msg.scan_id is not None:
                        ResolverResultsAgent.log().warning(
                            f"Delegated scan correlation_id {result_msg.correlation_id} indicated a soft failure with exit code {result_msg.resolver_exit_code}, scanning anyway."
                        )
                    else:
                        ResolverResultsAgent.log().error(
                            f"Delegated scan correlation_id {result_msg.correlation_id} indicated a hard failure with exit code {result_msg.resolver_exit_code}, no scan executed."
                        )
                        if result_msg.workflow == ScanWorkflow.PR:
                            await OrchestrationDispatch.dispatch_delegated_pr_scan_hard_failure_workflow(
                                ResolverResultsAgent.__orchestrator_factory(
                                    result_msg.details.orchestrator,
                                    result_msg.details.event_context,
                                )
                            )
                ...
                if result_msg.scan_id is not None:
                    ResolverResultsAgent.log().info(
                        await OrchestrationDispatch.dispatch_delegated_scan_workflow(
                            ResolverResultsAgent.__orchestrator_factory(
                                result_msg.details.orchestrator,
                                result_msg.details.event_context,
                            ),
