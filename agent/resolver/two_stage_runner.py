from agent.resolver.resolver_runner import (
    ResolverRunner,
...
class ResolverTwoStageExecutionContext(ResolverExecutionContext):
    ...
    def can_execute(self):
        return (
            self.__resolver_executor is not None
            and self.__resolver_executor.can_execute
        )
    ...
    async def execute_resolver(
        ...
        try:
            if self.__resolver_before:
                try:
                    resolver_result = await self.__resolver_executor.execute_resolver(
                        project_name, exclusions
                    )
                except subprocess.CalledProcessError as cpex:
                    ResolverExecutionContext.log().exception(cpex)
                    resolver_result = subprocess.CompletedProcess(
                        cpex.cmd, cpex.returncode, cpex.stdout
                    )

            exit_code = (
                resolver_result.returncode if resolver_result is not None else exit_code
            )

            try:
                exec_command = (
                    ResolverTwoStageExecutionContext.__docker_cmd
                    + self.__user_opts
                    + [
                        "-v",
                        f"{self.clone_path}:/code",
                        "-w",
                        "/code",
                        "--entrypoint",
                        self.__shell,
                ...
                shell_result = await AbstractRunner.execute_cmd_async(
                    exec_command, {"HOME": "/code"}
                )
            except subprocess.CalledProcessError as cpex:
                ResolverExecutionContext.log().exception(cpex)
            ...
            ResolverTwoStageExecutionContext.log().info(
                f"Resolver execution result code: {resolver_result.returncode}, "
