...
class ResolverToolkitExecutionContext(ResolverExecutionContext):
    ...
    def execution_resolver_out_file_path(self) -> str:
        return (
            "/sandbox/output/"
            + self.resolver_result_directory
            + "/"
            + self.resolver_result_filename
        )
    ...
    def execution_container_out_file_path(self) -> str:
        return (
            "/sandbox/output/"
            + self.container_result_directory
            + "/"
            + self.container_result_filename
        )

    def _get_resolver_exec_cmd(self) -> List[str]:
        return ResolverToolkitExecutionContext.__docker_cmd + [
            "-v",
            f"{self.clone_path}:/sandbox/input:ro",
            "-v",
            f"{self.work_root.name}:/sandbox/output",
...
class ResolverToolkitRunner(ResolverRunner):
    ...
    def __check_cmd_on_path_or_fail(cmd_name: str):
        if not os.path.exists(cmd_name) and os.path.isfile(cmd_name):
            raise ResolverAgentException(
                f"The required '{cmd_name}' command was not found on the path."
            )

    def __get_build_container_tag(self) -> str:
        build_args = [
            f"{self.__toolkit_path}autobuild.sh",
            "-t",
            self.__src_container_tag,
            "-d",
            self.__toolkit_path,
        ]
        ...
        resolver_tag = (
            (ResolverRunner.execute_cmd(build_args, {"HOME": self.home}))
            .stdout.decode()
            .rstrip("\n")
        )
    ...
    async def executor(self):
        return ResolverToolkitExecutionContext(
            self.work_path, self.resolver_opts, self.__run_container_tag
        )
