        ...
        if secret is not None:
            with SecretRegistry.__lock:
                if not secret in SecretRegistry.__secrets:
                    SecretRegistry.__secrets.append(
                        re.escape(secret.replace("\n", "").replace("\r", ""))
                    )
                    SecretRegistry.__compiled_regex = re.compile(
                        f"{SecretRegistry.__default_regex}|(?P<any>({'|'.join(SecretRegistry.__secrets)}))",
    ...
    def format(self, record):
        ...
        for secret in SecretRegistry.get_match_iter(msg):
            for m in [k for k in secret.groupdict().keys() if k in ["secret", "any"]]:
                msg = (
                    msg[0 : secret.start(m)]
                    + ("*" * (secret.end(m) - secret.start(m)))
                    + msg[secret.end(m) :]
                )
...
def bootstrap():
    ...
    log_cfg = {
        ...
        "loggers": {"root": {"handlers": ["console"], "level": get_log_level()}},
    }
