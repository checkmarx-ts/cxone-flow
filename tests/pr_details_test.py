...
class TestDetailRow(unittest.TestCase):
    ...
    def test_row_order(self):
        row = str(
            SortedDetailRows.DetailRow(
                ResultSeverity.CRITICAL, ["foo", "bar"], foo="1", bar="2"
            )
        )
    ...
    def test_row_order_by_kwarg(self):
        row = str(
            SortedDetailRows.DetailRow(
                ResultSeverity.CRITICAL, ["foo", "bar"], bar="2", foo="1"
            )
        )
        self.assertEqual(row, "| 1 | 2 |")
    ...
    def test_row_order_missing_arg(self):
        try:
            str(
                SortedDetailRows.DetailRow(
                    ResultSeverity.CRITICAL, ["foo", "bar"], bar="2"
                )
            )
            self.fail()
        except KeyError:
...
class TestSortedDetailRows(unittest.TestCase):
    ...
    def test_sort_by_severity_2xcomposite_key(self):
        rows = SortedDetailRows(
            ["foo", "bar"], lambda x: x.severity_rank_key + x["foo"] + x["bar"]
        )
    ...
    def test_sort_by_severity_composite_key_with_other_cols(self):
        rows = SortedDetailRows(
            ["foo", "bar"], lambda x: x.severity_rank_key + x["foo"]
        )
