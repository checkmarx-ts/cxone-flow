...
class TestAsymmetricSignatures(unittest.TestCase):
    ...
    def __get_pub_priv_by_name(test_name: str):
        return TestAsymmetricSignatures.__load_key_data(
            TestAsymmetricSignatures.__key_pair_tuples[test_name][
                TestAsymmetricSignatures.__priv_index
            ]
        ), TestAsymmetricSignatures.__load_key_data(
            TestAsymmetricSignatures.__key_pair_tuples[test_name][
                TestAsymmetricSignatures.__pub_index
            ]
        )
    ...
    def test_sig_fail_from_private(self):
        for test in TestAsymmetricSignatures.__key_pair_tuples.keys():
            with self.subTest(test):
                ...
                try:
                    obj.verify(
                        obj.sign(TestAsymmetricSignatures.__test_data),
                        TestAsymmetricSignatures.__test_data2,
                    )
                    self.assertTrue(False)
                except:
                    self.assertTrue(True)
        ...
        for test in TestAsymmetricSignatures.__key_pair_tuples.keys():
            with self.subTest(test):
                ...
                try:
                    obj.verify(
                        obj.sign(TestAsymmetricSignatures.__test_data),
                        TestAsymmetricSignatures.__test_data,
                    )
                    self.assertTrue(True)
                except:
                    self.assertTrue(False)
    ...
    def test_sig_from_public_success(self):
        for test in TestAsymmetricSignatures.__key_pair_tuples.keys():
            with self.subTest(test):
                ...
                try:
                    verifier.verify(
                        signer.sign(TestAsymmetricSignatures.__test_data),
                        TestAsymmetricSignatures.__test_data,
                    )
                    self.assertTrue(True)
                except:
    ...
    def test_sig_fail_from_public(self):
        for test in TestAsymmetricSignatures.__key_pair_tuples.keys():
            with self.subTest(test):
                ...
                try:
                    verifier.verify(
                        signer.sign(TestAsymmetricSignatures.__test_data2),
                        TestAsymmetricSignatures.__test_data,
                    )
                    self.assertTrue(False)
                except:
