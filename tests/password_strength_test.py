...
class TestPasswordStrength(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__remaining_length = 20 - len(
            self.__valid_uppercase + self.__valid_numbers + self.__valid_special
        )

    def test_combo_invalid_length(self):
        self.assertFalse(
            self.__shared_secret_policy.test(
                self.__valid_uppercase
                + self.__valid_numbers
                + self.__valid_special
                + "a" * (self.__remaining_length - 1)
            )
        )
