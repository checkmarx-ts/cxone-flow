...
class HeaderFilteredEventContext(EventContext):
    def __init__(self, raw_event_payload: str, headers: Dict, header_key_regex: str):
        ...
        EventContext.__init__(
            self,
            raw_event_payload=raw_event_payload,
            headers={k: headers[k] for k in headers if pattern.match(k)},
        )
...
class GithubAppAuthFactory(AuthFactory):
    ...
    def __find_install_id(self, event_context: EventContext):
        install_id_found = GithubAppAuthFactory.__event_installation_id.find(
            event_context.message
        )
        if len(install_id_found) == 0:
            install_id_found = GithubAppAuthFactory.__kickoff_installation_id.find(
                event_context.message
            )
            if len(install_id_found) == 0:
                raise AuthFactoryException(
                    "GitHub installation id was not found in the event payload."
                )
    ...
    def __find_app_id(self, event_context: EventContext):
        if GithubAppAuthFactory.__app_id_header in event_context.headers.keys():
            ...
            app_id_found = GithubAppAuthFactory.__kickoff_app_id.find(
                event_context.message
            )
        ...
        raise AuthFactoryException(
            f"GitHub AppId could not be determined from event header {GithubAppAuthFactory.__app_id_header} or in message body."
        )

    async def __get_token_tuple(
        self, event_context: EventContext = None, force_reauth: bool = False
    ):
        ...
        if install_id is None or app_id is None:
            raise AuthFactoryException(
                "Install Id and/or App Id is not in event payload, aborting."
            )
        ...
        async with GithubAppAuthFactory.__lock:
            if install_id in GithubAppAuthFactory.__token_cache.keys():
                ...
                if datetime.now(exp.tzinfo) >= exp:
                    GithubAppAuthFactory.log().debug(
                        f"Token for app_id {app_id} install_id {install_id} expired at {exp}"
                    )
                    token_tuple = None

            if token_tuple is None or force_reauth:
                GithubAppAuthFactory.log().debug(
                    f"Generating app token for app_id {app_id} install_id {install_id}"
                )
                token_response = json_on_ok(
                ...
                GithubAppAuthFactory.log().debug(
                    f"App token for app_id {app_id} install_id {install_id} generated."
                )

                token_tuple = (
                    SecretRegistry.register(token_response["token"]),
                    datetime.fromisoformat(token_response["expires_at"]),
                )
    ...
    async def get_auth(
        ...
        return HTTPBearerAuth(
            (await self.__get_token_tuple(event_context, force_reauth))[0]
        )
        ...
        async with self._lock:
            if self.__cached_token is not None and self.__token_exp is not None:
                if datetime.now(UTC) >= self.__token_exp:
                    ADOSPClientSecretAuthFactory.log().debug(
                        f"Service Principal token expired at {self.__token_exp}"
                    )
            ...
            if self.__cached_token is None or force_reauth:
                ...
                self.__cached_token = SecretRegistry.register(
                    token_response["access_token"]
                )
                self.__token_exp = datetime.now(UTC) + timedelta(
                    seconds=token_response["expires_in"]
                )
                ADOSPClientSecretAuthFactory.log().debug(
                    f"Service Principal created, expires at {self.__token_exp}."
                )
