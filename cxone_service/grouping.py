...
class GroupingService:
    ...
    def add_assignment_rule(self, clone_url_regex: str, group_paths: List[str]) -> None:
        self.__routes.append(
            Route(re.compile(clone_url_regex), [x.lstrip("/") for x in group_paths])
        )
        ...
        for path in group_paths:
            async with self.__lock:
                if path not in self.__groupid_index.keys():
                    ...
                    if (
                        path in self.__path_res_errors.keys()
                        and self.__path_res_errors[path]
                        > GroupingService.__MAX_RESOLUTION_ERRORS
                    ):
                        GroupingService.log().warning(
                            f"Group ID resolution for group {path} has failed to resolve in the last"
                            + f" {self.__path_res_errors[path]} attempts. The group may not be valid."
                        )

                    try:
                        group_defs = json_on_ok(
                            await retrieve_groups(self.__client, search=path)
                        )
                        ...
                        await self.__increment_error(path)
                        GroupingService.log().exception(
                            f"Exception trying to resolve id for group [{path}]", ex
                        )
