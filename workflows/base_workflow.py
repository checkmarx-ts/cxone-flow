...
class AbstractAsyncWorkflow:
    ...
    async def _publish(
        ...
        topic: str,
        msg: aio_pika.abc.AbstractMessage,
        log_msg: str,
        exchange: str,
    ) -> bool:
        try:
            ...
            if exchange:
                return AbstractAsyncWorkflow._log_publish_result(
                    await exchange.publish(msg, routing_key=topic), log_msg
                )
            else:
                AbstractAsyncWorkflow.log().error(
                    f"Client [{mq_client}] unable to retrieve exchange [{exchange}]"
                )
