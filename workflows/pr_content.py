...
class SortedDetailRows:
    ...
    def add_row(self, severity: ResultSeverity, **kwargs):
        self.__details.add(
            SortedDetailRows.DetailRow(severity, self.__header_order, **kwargs)
        )
...
class PullRequestAbstractMarkdownComment(PullRequestCommentContent):
    ...
    class PolicyViolationDetailRows(SortedDetailRows):
        def __init__(self):
            super().__init__(
                ["break_build_indicator", "policy_name", "policy_rule_name"],
                lambda x: x.severity_rank_key
                + x["policy_name"]
                + x["policy_rule_name"],
            )
    ...
    class SastDetailRows(SortedDetailRows):
        def __init__(self):
            super().__init__(
                ["severity_image_link", "issue", "source_permalink", "viewer_link"],
                lambda x: x.severity_rank_key + x["issue"],
            )
    ...
    class ScaDetailRows(SortedDetailRows):
        ...
        def __init__(self):
            super().__init__(
                ["severity_image_link", "cve", "package", "viewer_link"],
                lambda x: x.severity_rank_key
                + x["package_name"]
                + x["package_version"]
                + x["cve"],
    ...
    class IacDetailRows(SortedDetailRows):
        def __init__(self):
            super().__init__(
                ...
                lambda x: x.severity_rank_key + x["technology"],
            )

    class ResolvedDetailRows(SortedDetailRows):
        def __init__(self):
            super().__init__(
                ["severity_image_link", "name", "viewer_link"],
                lambda x: x.severity_rank_key + x["name"],
            )
    ...
    def comment_matches_identifier(comment_text: str):
        return PullRequestAbstractMarkdownComment.__comment_match.match(
    ...
    def __init__(self, server_base_url: str, overflow_link: str, scan_url: str):
        ...
        self.__elements = {
            PullRequestAbstractMarkdownComment.__identifier: [
                PullRequestAbstractMarkdownComment.__identifier
            ],
            PullRequestAbstractMarkdownComment.__header_begin: [
                PullRequestAbstractMarkdownComment.make_md_header_image(
        ...
        self.__resolved_detail_rows = (
            PullRequestAbstractMarkdownComment.ResolvedDetailRows()
        )
        self.__policy_detail_rows = (
            PullRequestAbstractMarkdownComment.PolicyViolationDetailRows()
        )
    ...
    @staticmethod
    def make_cxone_scan_url(
        cxone_display_url: str, project_id: str, scanid: str, branch: str
    ):
        path = (
            Path("projects")
            / Path(project_id)
            / Path(
                f"scans?id={scanid}&filter_by_Scan_Id={scanid}&branch={urllib.parse.quote_plus(branch)}"
            )
        )
        return f"{cxone_display_url}{path}"

    @staticmethod
    def make_cxone_md_scan_link(
        cxone_display_url: str,
        project_id: str,
        scanid: str,
        branch: str,
        link_text: str = None,
    ...
    @staticmethod
    def make_cxone_md_sca_result_link(
        cxone_display_url: str,
        project_id: str,
        scanid: str,
        title: str,
        ...
        display_path = (
            ...
            / Path(project_id)
            / Path(scanid)
            / Path(f"sca?internalPath=")
        )
        internal_path = (
            f"%2Fvulnerabilities%2F{cve}%253A{package_id}%2FvulnerabilityDetailsGql"
        )
    ...
    def make_md_header_image(server_base_url: str):
        return PullRequestAbstractMarkdownComment.make_md_image(
            PullRequestAbstractMarkdownComment._form_artifact_url(
                server_base_url, "checkmarx.png"
            ),
            "CheckmarxOne",
        )
    ...
    def make_md_severity_indicator(server_base_url: str, severity: str):
        img = (
            PullRequestAbstractMarkdownComment.__severity_map[severity.lower()]
            if severity.lower()
            in PullRequestAbstractMarkdownComment.__severity_map.keys()
            else PullRequestAbstractMarkdownComment.__default_severity
        )
        return PullRequestAbstractMarkdownComment.make_md_image(
            PullRequestAbstractMarkdownComment._form_artifact_url(server_base_url, img),
            severity,
        )

    def add_to_annotation(self, line: str):
        self.__elements[PullRequestAbstractMarkdownComment.__annotation_begin].append(
            line
        )

    def add_policy_violation_detail(
        self, policy_name: str, break_build: bool, policy_rule_name: str
    ):
        self.__policy_detail_rows.add_row(
            ResultSeverity(ResultSeverity.INFO),
            break_build_indicator=(
                PullRequestAbstractMarkdownComment._failure_unicode
                if break_build
                else PullRequestAbstractMarkdownComment.make_md_severity_indicator(
                    self.server_base_url, str(ResultSeverity.INFO)
                )
            ),
    ...
    def add_sast_detail(
        ...
        severity_image_link: str,
        issue: str,
        source_permalink: str,
        viewer_link: str,
    ):
        self.__sast_detail_rows.add_row(
    ...
    def start_policy_violation_detail_section(self):
        ...
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| Breaking | Policy Name | Policy Rule |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| :-: | - | - |"
        )
    ...
    def add_sca_detail(
        ...
        self.__sca_detail_rows.add_row(
            ResultSeverity(severity),
            severity_image_link=severity_image_link,
            cve=cve,
            ...
            package_version=PullRequestAbstractMarkdownComment.ScaDetailRows.normalized_version(
                package_version
            ),
        )
    ...
    def start_sca_detail_section(self):
        ...
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append("\n")
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| Severity | CVE | Package | Checkmarx Insight |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| :-: | - | - | - |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            self.__sca_detail_rows
        )
    ...
    def add_iac_detail(
        ...
        self.__iac_detail_rows.add_row(
            ResultSeverity(severity),
            severity_image_link=severity_image_link,
    ...
    def start_iac_detail_section(self):
        ...
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append("\n")
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| Severity | Technology | Source | Query | Checkmarx Insight |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| :-: | - | - | - | - |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            self.__iac_detail_rows
        )

    def add_resolved_detail(
        self,
        severity: ResultSeverity,
        severity_image_link: str,
        name: str,
        ...
        self.__resolved_detail_rows.add_row(
            ResultSeverity(severity),
            severity_image_link=severity_image_link,
            name=name,
            viewer_link=viewer_link,
    ...
    def start_resolved_detail_section(self):
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append("\n")
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "# Resolved SAST Results"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append("\n")
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| Severity | Name | Checkmarx Insight |"
        )
        self.__elements[PullRequestAbstractMarkdownComment.__details_begin].append(
            "| :-: | - | - |"
        )
    ...
    def start_summary_section(self, included_severities: List[ResultSeverity]):
        ...
        self.__elements[PullRequestAbstractMarkdownComment.__summary_begin].append(
            f"|--{"| :-: " * len(included_severities)}|"
        )

    def add_summary_entry(
        self,
        engine: str,
        counts_by_sev: Dict[ResultSeverity, str],
        included_severities: List[ResultSeverity],
    ):
    ...
    def __get_content(self, keys: List[str], char_limit: int) -> str:
        if char_limit < len(self.__overflow_link):
            raise ValueError(
                f"Content char limit of {char_limit} < overflow link size of {len(self.__overflow_link)}"
            )

        class Appender:
            def __init__(self, char_limit: int, overflow_link: str):
                self.__content = []
                self.__overflow_link = overflow_link
                self.__char_limit = char_limit
                self.__max = self.__char_limit - len(overflow_link)
                self.__total = 0
                self.__stop = False
            ...
            def append(self, v: str):
                ...
                if self.__total + len(v) >= self.__max:
                    self.__stop = True
                    self.__content.append(self.__overflow_link)
                    self.__total += len(self.__overflow_link)
                else:
                    self.__content.append(v)
                    self.__total += len(v)

        appender = Appender(char_limit, self.__overflow_link)
        ...
        for k in keys:
            ...
            if self.__elements[k] is not None:
                ...
                for item in self.__elements[k]:
                    if isinstance(item, SortedDetailRows):
                        for row in item:
                            if appender.stop:
                                break
                            term = "\n" if str(row)[-1:] != "\n" else ""
                            appender.append(str(row) + term)
                    else:
                        append_item = str(item)
                        term = "\n" if append_item[-1:] != "\n" else ""
                        appender.append(append_item + term)
...
class PrescanPullRequestMarkdownAnnotation(PullRequestCommentContent):
    ...
    def get_content(self, char_limit):
        md_content = PullRequestAbstractMarkdownComment.append_comment_identifier(
            f"*{self.__status_msg}*"
        )
...
class PullRequestMarkdownAnnotation(PullRequestAbstractMarkdownComment):
    def __init__(
        ...
        scan_link = PullRequestAbstractMarkdownComment.make_cxone_md_scan_link(
            cxone_display_url, project_id, scanid, branch
        )
        scan_url = PullRequestAbstractMarkdownComment.make_cxone_scan_url(
            cxone_display_url, project_id, scanid, branch
        )
...
class PullRequestMarkdownFeedback(PullRequestAbstractMarkdownComment):
    ...
    def __init__(
        self,
        excluded_severities: List[ResultSeverity],
        excluded_states: List[ResultStates],
        display_url: str,
        project_id: str,
        scanid: str,
        enhanced_report: dict,
        code_permalink_func: Callable,
        ...
        scan_link = PullRequestAbstractMarkdownComment.make_cxone_md_scan_link(
            display_url, project_id, scanid, pr_details.source_branch, "More results..."
        )
        scan_url = PullRequestAbstractMarkdownComment.make_cxone_scan_url(
            display_url, project_id, scanid, pr_details.source_branch
        )
    ...
    def __add_resolved_details(self, project_id: str):
        title_added = False
        for resolved in PullRequestMarkdownFeedback.__resolved_results_query.find(
            self.__enhanced_report
        ):
            for vuln in resolved.value["resolvedVulnerabilities"]:

                for result in vuln["resolvedResults"]:
                    if not PullRequestMarkdownFeedback.__test_in_enum(
                        ResultSeverity, result["severity"], self.__excluded_severities
                    ):
                        ...
                        if path_components[-1:].pop() == project_id:
                            ...
                            fixed_link = urllib.parse.urlunparse(
                                (
                                    parsed_url.scheme,
                                    parsed_url.netloc,
                        ...
                        self.add_resolved_detail(
                            ResultSeverity(result["severity"]),
                            PullRequestAbstractMarkdownComment.make_md_severity_indicator(
                                self.server_base_url, result["severity"]
                            ),
                            vuln["vulnerabilityName"],
                            PullRequestAbstractMarkdownComment.make_md_link(
                                fixed_link, "View"
                            ),
                        )
    ...
    def __add_iac_details(self, pr_details):
        title_added = False
        for result in PullRequestMarkdownFeedback.__iac_results_query.find(
            self.__enhanced_report
        ):
            x = result.value
            ...
            for query in x["queries"]:
                for result in query["resultsList"]:
                    if not (
                        PullRequestMarkdownFeedback.__test_in_enum(
                            ResultStates, result["state"], self.__excluded_states
                        )
                        or PullRequestMarkdownFeedback.__test_in_enum(
                            ResultSeverity,
                            result["severity"],
                            self.__excluded_severities,
                        )
                    ):
                        ...
                        self.add_iac_detail(
                            ...
                            x["name"],
                            f"`{result['fileName']}`{PullRequestAbstractMarkdownComment.make_md_link(
                                            self.__permalink(pr_details, result['fileName'], 1), "view")}",
                            query["queryName"],
                            PullRequestAbstractMarkdownComment.make_md_link(
                                result["resultViewerLink"], "Risk Details"
                            ),
                        )

    def __add_sca_details(self, display_url, project_id, scanid):
        title_added = False
        for result in PullRequestMarkdownFeedback.__sca_results_query.find(
            self.__enhanced_report
        ):
            x = result.value
            ...
            for category in x["packageCategory"]:
                for cat_result in category["categoryResults"]:
                    if not (
                        PullRequestMarkdownFeedback.__test_in_enum(
                            ResultStates, cat_result["state"], self.__excluded_states
                        )
                        or PullRequestMarkdownFeedback.__test_in_enum(
                            ResultSeverity,
                            cat_result["severity"],
                        ...
                        self.add_sca_detail(
                            ResultSeverity(cat_result["severity"]),
                            PullRequestAbstractMarkdownComment.make_md_severity_indicator(
                                self.server_base_url, cat_result["severity"]
                            ),
                            cat_result["cve"],
                            x["packageName"],
                            x["packageVersion"],
                            PullRequestAbstractMarkdownComment.make_cxone_md_sca_result_link(
                                display_url,
                                project_id,
                                scanid,
                                "Risk Details",
        ...
        if self.__policy_violations is not None and len(self.__policy_violations) > 0:
            ...
            for policy in self.__policy_violations:
                for rule_name in policy.ViolatedRules:
                    self.add_policy_violation_detail(
                        policy.PolicyName, policy.BreakBuild, rule_name
                    )

    def __add_sast_details(self, pr_details):
        title_added = False
        for result in PullRequestMarkdownFeedback.__sast_results_query.find(
            self.__enhanced_report
        ):

            x = result.value
            describe_link = PullRequestAbstractMarkdownComment.make_md_link(
                x["queryDescriptionLink"], x["queryName"]
            )
            for vuln in x["vulnerabilities"]:
                if not (
                    PullRequestMarkdownFeedback.__test_in_enum(
                        ResultStates, vuln["state"], self.__excluded_states
                    )
                    or PullRequestMarkdownFeedback.__test_in_enum(
                        ResultSeverity, vuln["severity"], self.__excluded_severities
                    )
                ):
                    ...
                    self.add_sast_detail(
                        ResultSeverity(vuln["severity"]),
                        PullRequestAbstractMarkdownComment.make_md_severity_indicator(
                            self.server_base_url, vuln["severity"]
                        ),
                        describe_link,
                        f"`{vuln['sourceFileName']}`;{PullRequestAbstractMarkdownComment.make_md_link(
                                        ...
                                        vuln['sourceLine'])}",
                        PullRequestAbstractMarkdownComment.make_md_link(
                            vuln["resultViewerLink"], "Attack Vector"
                        ),
                    )
    ...
    def __add_annotation_section(
        self, display_url: str, project_id: str, scanid: str, pr_details: PRDetails
    ):
        self.add_to_annotation(
            f"**Results for Scan ID {PullRequestAbstractMarkdownComment.make_cxone_md_scan_link(display_url, project_id, scanid, pr_details.source_branch)}**"
        )

        status_content = ""
        for engine_status in PullRequestMarkdownFeedback.__scanner_stat_query.find(
            self.__enhanced_report
        ):
    ...
    def __get_result_count_map(
        self, query_gen: Callable[[str], str]
    ) -> Dict[ResultSeverity, str]:
        counts = PullRequestMarkdownFeedback.__init_result_count_map()
        sev_incl = PullRequestMarkdownFeedback.__included_severities(
            self.__excluded_severities
        )
    ...
    def __add_sast_summary(self):
        ...
        self.add_summary_entry(
            ...
            self.__get_result_count_map(
                lambda sev_value: f"$.scanResults.resultsList[*].vulnerabilities[?(@.state!='Not Exploitable' & @.severity=='{sev_value}')]"
            ),
            sev_incl,
        )
    ...
    def __add_summary_section(self):
        ...
        self.start_summary_section(
            PullRequestMarkdownFeedback.__included_severities(
                self.__excluded_severities
            )
        )
