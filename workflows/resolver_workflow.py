...
from .messaging import (
    ...
    DelegatedScanMessageBase,
)
from api_utils.signatures import (
    AsymmetricSignatureSignerVerifier,
...
class ResolverScanningWorkflow(AbstractResolverWorkflow):
    ...
    def from_private_key(
        ...
        self.__signer = self.__verifier = (
            AsymmetricSignatureSignerVerifier.from_private_key(private_key)
        )
    ...
    def __DelegatedScanMessage_factory(
        self, msg: DelegatedScanMessage, retries: int
    ) -> aio_pika.Message:
        return self.__msg_factory(
            msg,
            headers={ResolverScanningWorkflow.__retry_header_key: retries},
            expiration=self.__scan_timeout_secs,
        )

    def __msg_factory(
        self, msg: DelegatedScanMessageBase, **kwargs
    ) -> aio_pika.Message:
        return self.__raw_msg_factory(msg.to_binary(), **kwargs)

    def __raw_msg_factory(self, msg: bytearray, **kwargs) -> aio_pika.Message:
        return aio_pika.Message(
            msg, delivery_mode=aio_pika.DeliveryMode.PERSISTENT, **kwargs
        )

    def get_signature(self, details: DelegatedScanDetails) -> bytearray:
        if self.__signer is None:
            raise WorkflowException(
                "The payload signature private key was not provided, this instance can't sign messages."
            )
    ...
    async def deliver_delegated_scan_outcome(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        route_key: str,
        msg: DelegatedScanResultMessage,
        exchange: str,
    ) -> bool:
        return await self._publish(
            mq_client,
            route_key,
            self.__msg_factory(msg),
            f"Delegated Scan Id {route_key}",
            exchange,
        )

    async def delegated_scan_kickoff(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        route_key: str,
        msg: DelegatedScanMessage,
        exchange: str,
    ) -> bool:
        if self.__no_kickoff:
            raise WorkflowException("This instance can't delegate a resolver scan.")

        return await self.delegated_scan_resubmit(
            mq_client, route_key, msg, exchange, self.__scan_retries
        )

    async def delegated_scan_resubmit(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        route_key: str,
        msg: DelegatedScanMessage,
        ...
        return await self._publish(
            mq_client,
            route_key,
            self.__DelegatedScanMessage_factory(msg, max(retries, 0)),
            f"Delegated Scan Workflow {route_key}",
            exchange,
        )

    async def get_delegated_scan_resubmit_count(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        msg: DelegatedScanMessage,
        headers: Dict,
