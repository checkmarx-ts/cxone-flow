...
class PushFeedbackService(CxOneFlowAbstractWorkflowService):
    ...
    class AbstractDeliveryAgent:
        ...
        async def execute_sarif_delivery(
            self, log: SarifLog, msg_headers: Dict
        ) -> None:
            raise NotImplementedError("execute_sarif_delivery")

        async def execute_sarif_error_delivery(
            self, msg: str, msg_headers: Dict
        ) -> None:
    ...
    class AbstractMessageDeliveryAgent(AbstractDeliveryAgent):
        ...
        def package_message(self, msg: bytes) -> Tuple[bytes, Dict[str, str]]:
            ...
            headers.update(
                PushFeedbackService.AbstractDeliveryAgent._gen_signature_headers(
                    self.__shared_secret, compressed_msg
                )
            )
    ...
    class AmqpDeliveryAgent(AMQPClient, AbstractMessageDeliveryAgent):
        def __init__(
            ...
            *args,
            ...
            PushFeedbackService.AbstractMessageDeliveryAgent.__init__(
                self, shared_secret
            )
        ...
        async def __publish_message(self, msg: bytes, headers: Dict):
            ...
            try:
                ...
                pub_result = await exchange.publish(
                    aio_pika.Message(
                        msg,
                        headers=headers,
                ...
                self.log().debug(
                    "Msg published for %s on exchange %s result: %s",
                    self.__moniker,
                    self.__dest_exchange,
                    pub_result,
                )
        ...
        async def execute_sarif_error_delivery(
            ...
            await self.__pack_and_publish(
                json.dumps({"error": msg}).encode("UTF-8"), msg_headers
            )
    ...
    class HttpDeliveryAgent(AbstractMessageDeliveryAgent):
        ...
        async def __post(self, msg: bytes, headers: Dict):
            ...
            while remaining >= 0:
                try:
                    if once:
                        PushFeedbackService.HttpDeliveryAgent.log().warning(
                            f"Delaying before retring delivery to {self.__url}. Retries remaining: {remaining}"
                        )
                    ...
                    PushFeedbackService.HttpDeliveryAgent.log().info(
                        f"Posted Sarif log to {self.__url} with response {response.status_code}"
                    )
            ...
            if not delivered:
                PushFeedbackService.HttpDeliveryAgent.log().error(
                    f"Delivery of Sarif log to {self.__url} failed!"
                )

        async def execute_sarif_delivery(
            self, log: SarifLog, msg_headers: Dict
        ) -> None:
            packaged_msg, packaged_headers = self.package_message(
                log.asjson().encode("UTF-8")
            )
            packaged_headers.update(msg_headers)
    ...
    async def execute_sarif_generation(
        self, msg: aio_pika.abc.AbstractIncomingMessage, cxone_client: CxOneClient
    ):
        ...
        try:
            if await self.__workflow.is_enabled():
                ...
                if not fm.is_error:
                    ...
                    sarif_log = await get_sarif_v210_log_for_scan(
                        cxone_client,
                        self.__sarif_opts,
                        fm.scanid,
                    ...
                    PushFeedbackService.log().debug(
                        f"Sarif log generated in {perf_counter_ns() - sarif_start}ns"
                    )
                    ...
                    await asyncio.wait(
                        [
                            asyncio.create_task(
                                agent.execute_sarif_delivery(sarif_log, headers)
                            )
                    ...
                    await asyncio.wait(
                        [
                            asyncio.create_task(
                                agent.execute_sarif_error_delivery(
                                    fm.error_msg, headers
                                )
                            )
                            for agent in self.__agents
                        ]
            ...
            PushFeedbackService.log().error(
                "Unrecoverable exception, aborting Sarif generation."
            )
            PushFeedbackService.log().exception(bex)
            await msg.ack()

    async def start_sarif_feedback(
        self, projectid: str, scanid: str, details: PushDetails
    ) -> None:
        if await self.__workflow.is_enabled():
            await self.__workflow.workflow_start(
                await self.mq_client(),
                self.__service_moniker,
                projectid,
                scanid,
                **(details.as_dict()),
            )
    ...
    async def handle_completed_scan(self, msg: ScanAwaitMessage) -> None:
        if msg.workflow == ScanWorkflow.PUSH:
            await self.__workflow.feedback_start(
                await self.mq_client(),
                msg.moniker,
                msg.projectid,
                msg.scanid,
    ...
    async def handle_awaited_scan_error(
        ...
        if msg.workflow == ScanWorkflow.PUSH:
            await self.__workflow.feedback_error(
                await self.mq_client(),
                msg.moniker,
                msg.projectid,
                msg.scanid,
