...
class PushWorkflow(AbstractFeedbackWorkflow):
    ...
    def __init__(
        self, enabled: bool = False, interval_seconds: int = 60, scan_timeout: int = 48
    ):
    ...
    def __feedback_gen_msg_factory(
        self,
        moniker: str,
        projectid: str,
        scanid: str,
    ...
    async def __publish_feedback_msg(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        msg: ScanFeedbackMessage,
        msg_type: str,
    ) -> None:
        topic = PushFeedbackService.make_topic(
            ScanStates.FEEDBACK, FeedbackWorkflow.PUSH_GEN, msg.moniker
        )
        await self._publish(
            mq_client,
            topic,
            ...
            f"{msg_type}: {topic} for scan id {msg.scanid} on service {msg.moniker}",
    ...
    async def feedback_start(
        ...
        **kwargs,
        ...
        await self.__publish_feedback_msg(
            mq_client,
            self.__feedback_gen_msg_factory(moniker, projectid, scanid, **kwargs),
            "Sarif Gen",
        )
    ...
    async def feedback_error(
        ...
        **kwargs,
        ...
        await self.__publish_feedback_msg(
            mq_client,
            self.__feedback_gen_msg_factory(
                moniker, projectid, scanid, True, error_msg, **kwargs
            ),
            "Sarif Gen Error",
        )
    ...
    async def workflow_start(
        ...
        **kwargs,
        ...
        topic = PushFeedbackService.make_topic(
            ScanStates.AWAIT, FeedbackWorkflow.PUSH_GEN, moniker
        )
        await self._publish(
            mq_client,
            topic,
            ...
            f"Sarif workflow start: {topic} for scan id {scanid} on service {moniker}",
            CxOneFlowAbstractWorkflowService.EXCHANGE_SCAN_INPUT,
        )
