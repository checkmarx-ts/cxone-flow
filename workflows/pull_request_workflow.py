...
from workflows.feedback_services.pr import PRFeedbackService
from workflows.enums import ScanWorkflow, ScanStates, ResultSeverity, ResultStates
...
class PullRequestWorkflow(AbstractPRFeedbackWorkflow):
    ...
    def __feedback_error_msg_factory(
        self,
        projectid: str,
        scanid: str,
        moniker: str,
    ...
    def __prescan_annotation_msg_factory(
        self, moniker: str, annotation: str, **kwargs
    ) -> aio_pika.Message:
        return aio_pika.Message(
            PreScanAnnotationMessage.factory(
                moniker=moniker,
                annotation=annotation,
                state=ScanStates.PRESCAN,
                workflow=ScanWorkflow.PR,
                workflow_details=kwargs,
            ).to_binary(),
            delivery_mode=aio_pika.DeliveryMode.PERSISTENT,
    ...
    async def workflow_start(
        ...
        **kwargs,
        ...
        await self._publish(
            mq_client,
            topic,
            self.__await_msg_factory(projectid, scanid, moniker, **kwargs),
            f"{topic} for scan id {scanid} on service {moniker}",
    ...
    async def feedback_error(
        ...
        **kwargs,
        ...
        topic = PRFeedbackService.make_topic(
            ScanStates.FAILURE, ScanWorkflow.PR, moniker
        )
        await self._publish(
            mq_client,
            topic,
            self.__feedback_error_msg_factory(
                projectid, scanid, moniker, True, error_msg, **kwargs
            ),
            f"{topic} for scan id {scanid} on service {moniker}",
            CxOneFlowAbstractWorkflowService.EXCHANGE_SCAN_INPUT,
        )
    ...
    async def feedback_start(
        ...
        **kwargs,
        ...
        topic = PRFeedbackService.make_topic(
            ScanStates.FEEDBACK, ScanWorkflow.PR, moniker
        )
        await self._publish(
            mq_client,
            topic,
            self.__feedback_msg_factory(projectid, scanid, moniker, **kwargs),
            f"{topic} for scan id {scanid} on service {moniker}",
            CxOneFlowAbstractWorkflowService.EXCHANGE_SCAN_INPUT,
        )

    async def annotation_start(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        moniker: str,
        projectid: str,
        ...
        **kwargs,
        ...
        topic = PRFeedbackService.make_topic(
            ScanStates.ANNOTATE, ScanWorkflow.PR, moniker
        )
        await self._publish(
            mq_client,
            topic,
            self.__annotation_msg_factory(
                projectid, scanid, moniker, annotation, **kwargs
            ),
            f"{topic} for scan id {scanid} on service {moniker}",
            CxOneFlowAbstractWorkflowService.EXCHANGE_SCAN_INPUT,
        )

    async def prescan_annotation_start(
        self,
        mq_client: aio_pika.abc.AbstractRobustConnection,
        moniker: str,
        annotation: str,
        **kwargs,
        ...
        topic = PRFeedbackService.make_topic(
            ScanStates.PRESCAN, ScanWorkflow.PR, moniker
        )
        await self._publish(
            mq_client,
            topic,
            self.__prescan_annotation_msg_factory(moniker, annotation, **kwargs),
            f"{topic} on service {moniker}",
            CxOneFlowAbstractWorkflowService.EXCHANGE_SCAN_INPUT,
        )
