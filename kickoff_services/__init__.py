...
class KickoffService:
    ...
    async def get_completed_ko_scans_by_project(
        ...
        async for scan in page_generator(
            ...
            tags_keys=KickoffService.__TAG_KEY,
            tags_values=self.__moniker,
            statuses=["Running", "Queued", "Completed"],
        ):
            scans.append(
                ExecutingScan(
                    project_name, scan["projectId"], scan["id"], scan["branch"]
                )
            )
    ...
    async def get_running_ko_scans(self) -> List[ExecutingScan]:
        ...
        async for scan in page_generator(
            ...
            scans.append(
                ExecutingScan(
                    scan["projectName"], scan["projectId"], scan["id"], scan["branch"]
                )
            )
...
class DummyKickoffService(KickoffService):
    ...
    async def get_completed_ko_scans_by_project(
        self, client: CxOneClient, project_name: str
    ) -> List[ExecutingScan]:
        return []
