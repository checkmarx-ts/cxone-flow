...
service definition.  As an example, most regular expressions should be in a format similar to
the example below:
...
For cloud-hosted SCM services, it is highly recommended to add elements to the regular
expression that match only your subscribed organization units.  This will ensure
that events from other subscribers are discarded if received 
\footnote{The probability of another subscriber sending events to your \cxoneflow instance
are low but it is not infeasible since the event sources come from the same cloud host.}
by \cxoneflowns. In Azure DevOps,
for example, add collection names at the end of the regular expression. BitBucket Cloud
clone URLs also contain the workspace; each workspace in the subscriber account can be
added to the \texttt{repo-match} regular expression.
