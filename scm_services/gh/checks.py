...
class GHServiceChecks(AbstractGHService, PolicyProperties):
    ...
    __check_req_action_pr_head_sha_query = parse(
    ...
    def _get_head_sha(self, pr_details: PRDetails) -> Union[str, None]:
        found = GHServiceChecks.__check_req_action_pr_head_sha_query.find(
    ...
    async def __find_running_check(
        self, pr_details: PRDetails, status: str
    ) -> Union[None, int]:
        ...
        query = {"check_name": self.check_name, "status": status}

        try:
            check_data = json_on_ok(
                await self.exec(
                    "GET",
                    url,
                    query=query,
            ...
            GHServiceChecks.log().error(
                "%s (The Github app may not have permission to read and write Checks)",
                ex,
            )

    async def __update_run(
        self, pr_details: PRDetails, payload: Dict, run_id: int
    ) -> None:
    ...
    async def __create_run(
        self, pr_details: PRDetails, payload: Dict
    ) -> Union[int, None]:
        url = f"/repos/{pr_details.organization}/{pr_details.repo_slug}/check-runs"

        try:
            create_data = json_on_ok(
                await self.exec(
                    "POST",
                    url,
                    extra_headers={
    ...
    async def exec_pr_scan_update_decorate(
        ...
        GHServiceChecks.log().debug(
            "Check run ID %d updated for scan id %s", run_id, scan_details.scanid
        )
    ...
    async def exec_pr_scan_pending_decorate(
        ...
        if run_id is not None:
            GHServiceChecks.log().debug(
                "Pending scan check run ID %d created for PR #%s",
                run_id,
                pr_details.pr_id,
            )
        else:
            GHServiceChecks.log().warning(
                "No pending scan check created for PR #%s", pr_details.pr_id
            )
    ...
    async def exec_pr_scan_failure_decorate(
        ...
        if run_id is None:
            GHServiceChecks.log().warning(
                "No running check found in PR#%s for scan %s, no check updated.",
                pr_details.pr_id,
                scan_details.scanid,
            )
        else:
            payload = {
                ...
                "conclusion": "failure",
                "output": {
                    "title": content.get_status_msg(
                        GHServiceChecks.__max_content_chars
                    ),
                    "summary": GHServiceChecks.__summary_text,
                    "text": content.get_content(GHServiceChecks.__max_content_chars),
                },
    ...
    async def exec_pr_prescan_failure(self, pr_details: PRDetails, fail_msg: str):
        ...
        if run_id is None:
            GHServiceChecks.log().warning(
                "No queued prescan check found in PR#%s, no check updated.",
                pr_details.pr_id,
            )
        else:
            payload = {
