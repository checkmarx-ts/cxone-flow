...
from workflows.pr_content import (
    ...
    PullRequestAbstractMarkdownComment,
)
...
class GHServiceCommitStatus(GHServiceBasic, PolicyProperties):
    ...
    def __make_payload(self, state: str, content: PullRequestCommentContent) -> Dict:
        return {
            ...
            "description": content.get_status_msg(
                GHServiceCommitStatus.__status_msg_max
            ),
    ...
    async def __update_status(
        self, state: str, pr_details: PRDetails, content: PullRequestCommentContent
    ) -> None:
        api_url = self.__make_api_url(pr_details)
        resp = await self.__post_update(pr_details, self.__make_payload(state, content))

        if not resp.ok:
            GHServiceCommitStatus.log().error(
                "Commit status update failed: %s returned %d:%s",
                api_url,
                resp.status_code,
                resp.text,
            )
        else:
            GHServiceCommitStatus.log().debug(
                "Commit status updated: %s: %s", api_url, json_on_ok(resp)
            )
    ...
    async def exec_pr_unrecoverable_error(
        ...
        await self.__post_update(
            pr_details, self.__make_plaintext_payload("error", fail_msg)
        )
