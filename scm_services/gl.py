...
class GLService(SCMService):
    ...
    async def __create_or_update_pr_comment(
        self, pr_details: PRDetails, content: Union[PullRequestCommentContent, str]
    ):
        pr_api_params = {
        ...
        existing_comments = json_on_ok(
            await self.exec("GET", GLService.__notes_api_path, url_vars=pr_api_params)
        )
        ...
        for comment in existing_comments:
            if PullRequestAbstractMarkdownComment.comment_matches_identifier(
                ...
                self.log().info(
                    f"Updating comment {comment['id']} for PR#{pr_details.pr_id} on {pr_details.repo_slug}"
                )
        ...
        if isinstance(content, PullRequestCommentContent):
            ...
            content_to_send = (
                PullRequestAbstractMarkdownComment.append_comment_identifier(content)
            )

        posted = json_on_ok(
            await self.exec(
                method, note_url, url_vars=pr_api_params, body={"body": content_to_send}
            )
        )
    ...
    def create_code_permalink(
        ...
        return form_url(
            self.display_url,
            f"/{pr_details.repo_slug}/-/blob/{pr_details.source_branch}{code_path}",
            f"L{code_line}",
        )
