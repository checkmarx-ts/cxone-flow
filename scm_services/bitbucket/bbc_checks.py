...
class BBCServiceChecks(BBCServiceBasic, PolicyProperties):
    ...
    async def __update_check(
        self, state: str, pr_details: PRDetails, description: str, url: str
    ):
        payload = {
        ...
        await self.exec(
            "POST",
            f"/repositories/{pr_details.organization}/{pr_details.repo_slug}/commit/{pr_details.source_hash}/statuses/build",
    ...
    async def exec_pr_scan_update_decorate(
        ...
        await BBCServiceBasic.exec_pr_scan_update_decorate(
            self, pr_details, content, scan_details
        )
    ...
    async def exec_pr_scan_failure_decorate(
        ...
        await BBCServiceBasic.exec_pr_scan_failure_decorate(
            self, pr_details, content, scan_details
        )
    ...
    async def exec_pr_scan_success_decorate(
        ...
        await BBCServiceBasic.exec_pr_scan_success_decorate(
            self, pr_details, content, scan_details
        )
    ...
    async def exec_pr_unrecoverable_error(
        ...
        await self.__update_check(
            ...
            pr_details,
            fail_msg,
            PullRequestAbstractMarkdownComment.make_cxone_scan_url(
                self.services.cxone.display_link,
        ...
        await BBCServiceBasic.exec_pr_unrecoverable_error(
            self, pr_details, scan_details, fail_msg
        )

    async def exec_pr_prescan_failure(self, pr_details: PRDetails, fail_msg: str):
        await self.__update_check(
            "FAILED", pr_details, fail_msg, self.services.cxone.display_link
        )
