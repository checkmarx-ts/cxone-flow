...
class CloneWorker:
    ...
    async def loc(self) -> str:
        try:
            ...
            if (
                CloneWorker.__stderr_auth_fail.match(
                    ex.stderr.decode("UTF-8").replace("\n", "")
                )
                and ex.returncode == CloneWorker.__auth_fail_exit_code
            ):
                raise CloneAuthException(ex.stderr.decode("UTF-8"))
            else:
                self.__log.error(
                    f"{ex} stdout: [{ex.stdout.decode('UTF-8')}] stderr: [{ex.stderr.decode('UTF-8')})]"
                )
                raise
...
class Cloner:
    ...
    def using_basic_auth(
        username: str, password: str, ssl_no_verify: bool, in_header: bool = False
    ):
        Cloner.log().debug("Clone config: using_basic_auth")

        if not in_header:
            retval = BasicAuthWithCredsInUrl(
                username, SecretRegistry.register(password), ssl_no_verify
            )
            retval.__git_cmd_stub = ["git"]
        else:
            retval = Cloner(ssl_no_verify)
            encoded_creds = SecretRegistry.register(
                base64.b64encode(f"{username}:{password}".encode("UTF8")).decode("UTF8")
            )
            retval.__git_cmd_stub = [
                "git",
                "-c",
                f"http.extraHeader=Authorization: Basic {encoded_creds}",
            ]
    ...
    def using_ssh_auth(ssh_private_key_file: Path, ssh_port: int):
        ...
        with open(ssh_private_key_file, "rt") as source:
            with tempfile.NamedTemporaryFile(
                mode="wt", delete_on_close=False, delete=False
            ) as dest:
                shutil.copyfileobj(source, dest)
                retval.__keyfile = dest.file.name

        retval.__additional_env["GIT_SSH_COMMAND"] = (
            f"ssh -i '{shlex.quote(retval.__keyfile)}' -oIdentitiesOnly=yes -oStrictHostKeyChecking=accept-new -oHostKeyAlgorithms=+ssh-rsa -oPubkeyAcceptedAlgorithms=+ssh-rsa"
        )
        retval.__git_cmd_stub = ["git"]
    ...
    @staticmethod
    def using_github_app_auth(
        gh_auth_factory: GithubAppAuthFactory, ssl_no_verify: bool
    ):
    ...
    @staticmethod
    def using_ado_sp_client_secret_auth(
        auth_factory: ADOSPAuthFactory, ssl_no_verify: bool
    ):
    ...
    @staticmethod
    def do_clone(
        run_env: Dict, git_cmd_stub: List[str], clone_url: str, clone_output_loc: str
    ) -> bool:

        def run(cmd, cwd=None):
            Cloner.log().debug(f"Executing: {cmd}")
            result = subprocess.run(
                cmd, capture_output=True, env=run_env, check=True, cwd=cwd
            )
            Cloner.log().debug(
                f"git task return code [{result.returncode}] stdout: [{result.stdout}] stderr: [{result.stderr}]"
            )
            return result
        ...
        if clone_result.returncode == 0:
            try:
                ...
                Cloner.log().warning(
                    f"Sub-modules were not initialized properly for repo {clone_url}, scan will not include all git submodules."
                )
    ...
    async def clone(
        ...
        fixed_clone_url = await self._fix_clone_url(
            clone_url, event_context, force_reauth
        )
        temp_dir_object = (
            tempfile.TemporaryDirectory(delete=False, prefix=temp_root)
            if make_temp
            else None
        )
        ...
        thread = asyncio.to_thread(
            Cloner.do_clone,
            run_env=dict(self.__running_env),
            git_cmd_stub=list(await self._git_cmd_stub(event_context, force_reauth)),
            clone_url=fixed_clone_url,
            clone_output_loc=clone_output_loc,
        )
    ...
    async def reset_head(self, code_path, hash):
        try:
            result = await asyncio.to_thread(
                ...
                ["git", "reset", "--hard", hash],
                capture_output=True,
                env=self.__running_env,
                check=True,
                cwd=code_path,
            )

            self.log().debug(
                f"Reset task: return code [{result.returncode}] stdout: [{result.stdout}] stderr: [{result.stderr}]"
            )
    ...
    async def _git_cmd_stub(
        self, event_context: EventContext = None, force_reauth: bool = False
    ) -> List:
...
class GithubAppCloner(AuthFactoryCloner):
    ...
    async def _fix_clone_url(
        ...
        token = SecretRegistry.register(
            await self._auth_factory.get_token(event_context, force_reauth)
        )
