...
class ADOEServiceChecks(ADOEServiceBasic):
    ...
    def __make_payload(
        self, state: str, scan_url: str, msg: str, iteration: int
    ) -> Dict:
        return (
            {
                ...
                "context": {"name": self.__check_name, "genre": self.__check_genre},
            }
            | {"targetUrl": scan_url}
    ...
    async def __get_iteration(self, pr_details: PRDetails) -> int:
        ...
        iteration_target_hash = (
            ADOEServiceChecks.__pr_iteration_hash.find(pr_details.event_context.message)
            .pop()
            .value
        )
        matching_iteration_list = parser.parse(
            f'$.value[?(@.sourceRefCommit.commitId == "{iteration_target_hash}")].id'
        ).find(iterations_list)
    ...
    async def __set_check_status(
        self, pr_details: PRDetails, state: str, scan_url: str = None, msg: str = None
    ):
        ...
        try:
            resp = await self.exec(
                "POST",
                status_create_api_url,
                extra_headers={"Content-Type": "application/json"},
                body=json.dumps(
                    self.__make_payload(
                        state, scan_url, msg, await self.__get_iteration(pr_details)
                    )
                ),
            )

            if not resp.ok:
                ADOEServiceChecks.log().error(
                    "Attempt to update check "
                    + f"{self.__get_check_name()} at {status_create_api_url} "
                    + f"failed with response: {resp.status_code}: {resp.text}"
                )
        except SCMAuthException:
            ADOEServiceChecks.log().warning(
                f"Could not create status, permissions for Code:R+W are required to update status checks: {status_create_api_url}"
            )
    ...
    async def exec_pr_scan_update_decorate(
        ...
        await self.__set_check_status(
            pr_details,
            "pending",
            content.scan_url,
            content.get_status_msg(ADOEServiceChecks.__MAX_CHECK_STRING_LEN),
        )
        return await super().exec_pr_scan_update_decorate(
            pr_details, content, scan_details
        )
    ...
    async def exec_pr_scan_pending_decorate(
        ...
        await self.__set_check_status(
            ...
            "pending",
            content.scan_url,
            msg=content.get_status_msg(ADOEServiceChecks.__MAX_CHECK_STRING_LEN),
        )
    ...
    async def exec_pr_scan_failure_decorate(
        ...
        return await super().exec_pr_scan_failure_decorate(
            pr_details, content, scan_details
        )
    ...
    async def exec_pr_scan_success_decorate(
        ...
        return await super().exec_pr_scan_success_decorate(
            pr_details, content, scan_details
        )
    ...
    async def exec_pr_unrecoverable_error(
        ...
        return await super().exec_pr_unrecoverable_error(
            pr_details, scan_details, fail_msg
        )
