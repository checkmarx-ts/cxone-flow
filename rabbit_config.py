...
async def setup() -> None:
    ...
    for moniker in monikers:
        ...
        async with rmq.channel() as channel:
            ...
            pr_feedback_queue = await channel.declare_queue(WorkflowStateService.get_pr_feedback_queue_name(moniker), durable=True)
            pr_annotate_queue = await channel.declare_queue(WorkflowStateService.get_pr_annotate_queue_name(moniker), durable=True)
            ...
            await pr_feedback_queue.bind(scan_feedback_exchange, WorkflowStateService.get_pr_feedback_binding_topic(moniker))
            await pr_annotate_queue.bind(scan_annotate_exchange, WorkflowStateService.get_pr_annotate_binding_topic(moniker))
