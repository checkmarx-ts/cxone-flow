...
class AbstractOrchestrator:
    ...
    def __get_path_dict(path: str, root: str = None) -> dict:
        ...
        for entry in p.iterdir():
            if entry.is_dir():
                ...
                AbstractOrchestrator.log().warning(
                    f"File skipped: {str(entry)} (Symlink: {entry.is_symlink()})"
                )
        return return_dict
    ...
    def __zip_write_delegate(zip_entries: Dict, zipfile: zipfile.ZipFile):
        try:
            for entry_key in zip_entries.keys():
                AbstractOrchestrator.log().debug(
                    f"Writing file [{zip_entries[entry_key]}] @ {zipfile.fp.tell()}"
                )
                zipfile.write(entry_key, zip_entries[entry_key])
        except ValueError as vex:
            AbstractOrchestrator.log().exception(
                "ValueError exception indicating 'write to file that is closed'"
                + " indicates the MQ is timing out waiting for message ACK.  You may need to increase the"
                + " timeout for consumed message acknowledgements.",
                vex,
            )
            raise
    ...
    async def exec_local_scan(
        ...
        tags: dict,
    ) -> Tuple[ScanInspector, ScanAction]:

        check = perf_counter_ns()

        with tempfile.NamedTemporaryFile(suffix=".zip") as zip_file:
            with zipfile.ZipFile(
                zip_file, mode="w", compression=zipfile.ZIP_DEFLATED, compresslevel=9
            ) as upload_payload:
                zip_entries = AbstractOrchestrator.__get_path_dict(code_path)

                AbstractOrchestrator.log().debug(
                    f"[{scan_source_msg}] zipping {len(zip_entries)} files for scan."
                )

                await asyncio.to_thread(
                    AbstractOrchestrator.__zip_write_delegate,
                    zip_entries,
                    upload_payload,
                )

                AbstractOrchestrator.log().info(
                    f"[{scan_source_msg}] zipped {len(zip_entries)} file in {perf_counter_ns() - check}ns"
                )

            scan_submit = await cxone_service.execute_scan(
                zip_file.name, project_config, source_branch, tags
            )

            AbstractOrchestrator.log().debug(scan_submit)
            AbstractOrchestrator.log().info(
                f"Scan id {scan_submit['id']} created for [{scan_source_msg}]"
            )

            return ScanInspector(scan_submit), AbstractOrchestrator.ScanAction.EXECUTING

    @staticmethod
    async def exec_clone_scan(
        cxone_service: CxOneService,
        ...
        while clone_auth_fails <= 1:
            try:
                async with await scm_service.cloner.clone(
                    clone_url, event_context, clone_auth_fails > 0
                ) as clone_worker:
                    ...
                    AbstractOrchestrator.log().info(
                        f"{clone_url} cloned in {perf_counter_ns() - check}ns"
                    )

                    return await AbstractOrchestrator.exec_local_scan(
                        code_path,
                        cxone_service,
    ...
    async def __orchestrate_scan(
        self, services: CxOneFlowServices, scan_tags: dict, workflow: ScanWorkflow
    ) -> Tuple[ScanInspector, ScanAction]:
        ...
        if target_branch in protected_branches:
            AbstractOrchestrator.log().info(
                f"Scan workflow executing for {clone_url}:{source_hash}:{source_branch} -> {target_branch}"
            )

            project_config = await services.cxone.load_project_config(
                await self.get_default_cxone_project_name(),
            ...
            if not self.delegated_scan:
                try:
                    resolver_tag = await services.cxone.get_resolver_tag_for_project(
                        project_config,
                        services.resolver.project_tag_key,
                        services.resolver.default_tag,
                    )
                    if resolver_tag is not None:
                        if await services.resolver.request_resolver_scan(
                            resolver_tag,
                            project_config,
                            services.scm,
                            services.cxone,
                            ...
                            scan_tags,
                            workflow,
                            self.__event_context,
                            f"{self.__class__.__module__}.{self.__class__.__name__}",
                        ):
                            return None, AbstractOrchestrator.ScanAction.DELEGATED
                        else:
                            AbstractOrchestrator.log().warning(
                                f"Delegated scan request failed for tag {resolver_tag} but proceeding with scanning."
                            )

                except WorkflowException as ex:
                    # pylint: disable=E1205
                    AbstractOrchestrator.log().exception(
                        "Delegated scan workflow exception.", ex
                    )

            inspector, action = await AbstractOrchestrator.exec_clone_scan(
                services.cxone,
                services.scm,
                clone_url,
                source_hash,
            ...
            AbstractOrchestrator.log().info(
                f"{clone_url}:{source_hash}:{source_branch} is not related to any protected branch: {protected_branches}"
            )
    ...
    async def _execute_delegated_push_scan_workflow(
        ...
        if inspector.executing:
            ...
            await services.push.start_sarif_feedback(
                inspector.project_id,
                inspector.scan_id,
                PushDetails.factory(
                    clone_url=self._repo_clone_url(services.scm.cloner),
    ...
    async def _execute_push_scan_workflow(
        ...
        inspector, action = await self.__orchestrate_scan(
            services, submitted_scan_tags, ScanWorkflow.PUSH
        )

        if (
            inspector is not None
            and action == AbstractOrchestrator.ScanAction.EXECUTING
        ):
            source_branch, commit_hash = await self._get_source_branch_and_hash()
            clone_url = self._repo_clone_url(services.scm.cloner)
            await services.push.start_sarif_feedback(
    ...
    async def __start_pr_workflow(
        self, services: CxOneFlowServices, inspector: ScanInspector
    ):
        await services.pr.start_pr_scan_workflow(
            inspector.project_id,
            inspector.scan_id,
    ...
    async def __start_delegated_pr_workflow(self, services: CxOneFlowServices):
        await services.pr.start_delegated_pr_scan_workflow(
            await self._make_prdetails(services), services.cxone, services.scm
        )
    ...
    async def _execute_delegated_pr_scan_workflow(
        ...
        await self.__start_pr_workflow(
            services, await services.cxone.load_scan_inspector(scan_id)
        )
    ...
    async def _execute_pr_scan_workflow(
        ...
        inspector, scan_action = await self.__orchestrate_scan(
            services, submitted_scan_tags, ScanWorkflow.PR
        )
        if (
            inspector is not None
            and scan_action == AbstractOrchestrator.ScanAction.EXECUTING
        ):
            ...
            AbstractOrchestrator.log().info(
                f"PR workflow delegated for PR {self._pr_id}."
            )
        else:
            AbstractOrchestrator.log().warning(
                f"No scan returned, PR workflow not started for PR {self._pr_id}."
            )
    ...
    async def _execute_pr_tag_update_workflow(self, services: CxOneFlowServices):
        ...
        updated_scans = await services.cxone.update_scan_pr_tags(
            await services.naming.get_project_name(
                await self.get_default_cxone_project_name(), self.event_context
            ),
            self._pr_id,
