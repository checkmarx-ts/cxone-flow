...
class OrchestrationDispatch:
    ...
    async def execute(orchestrator: AbstractOrchestrator):
        ...
        try:
            OrchestrationDispatch.log().debug(
                f"Service lookup: {orchestrator.route_urls}"
            )
            services = CxOneFlowConfig.retrieve_services_by_route(
                orchestrator.route_urls, orchestrator.config_key
            )
            if services is None:
                OrchestrationDispatch.log().error(
                    f"No configured services for SCM: {orchestrator.config_key}"
                )
                return

            OrchestrationDispatch.log().debug(
                f"Service lookup success: {orchestrator.route_urls}"
            )

            if await orchestrator.is_signature_valid(services.scm.shared_secret):
                return await orchestrator.execute(services)
            else:
                OrchestrationDispatch.log().warning(
                    f"Payload signature validation failed, webhook payload ignored."
                )
        except RouteNotFoundException:
            OrchestrationDispatch.log().warning(
                f"Event [{orchestrator.event_name}] not handled for SCM [{orchestrator.config_key}]"
            )
        except CxOneException as ex:
            OrchestrationDispatch.log().warning(
                f"Event [{orchestrator.event_name}] not handled for SCM [{orchestrator.config_key}] due to Checkmarx One error: {ex}"
            )

    @staticmethod
    async def dispatch_delegated_scan_workflow(
        orchestrator: AbstractOrchestrator, scan_id: str
    ):
        try:
            OrchestrationDispatch.log().debug(
                f"Service lookup: {orchestrator.route_urls}"
            ...
            OrchestrationDispatch.log().warning(
                f"Delegated scan for [{orchestrator.event_name}] not handled for SCM [{orchestrator.config_key}]"
            )

    @staticmethod
    async def dispatch_delegated_pr_scan_hard_failure_workflow(
        ...
        try:
            ...
            return await orchestrator.handle_delegated_pr_scan_hard_fail(
                services, f"The prescan operation failed to execute properly."
            )
        except RouteNotFoundException as ex:
            OrchestrationDispatch.log().warning(
                f"Delegated scan for [{orchestrator.event_name}] not handled for SCM [{orchestrator.config_key}]"
    ...
    async def execute_kickoff(orchestrator: KickoffOrchestrator) -> bool:
        try:
            ...
            if not await orchestrator.valid_bearer_token(services.kickoff):
                OrchestrationDispatch.log().error(
                    f"{services.cxone.moniker}: Invalid bearer token sent for kickoff scan with SCM [{orchestrator.config_key}]"
                )
