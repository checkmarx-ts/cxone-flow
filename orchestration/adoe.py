...
class AzureDevOpsEnterpriseOrchestrator(AbstractOrchestrator):
    ...
    def __init__(self, event_context: EventContext):
        ...
        self.__isdiagnostic = AzureDevOpsEnterpriseOrchestrator.__diag_id in [
            x.value
            for x in list(
                AzureDevOpsEnterpriseOrchestrator.__diagid_query.find(
        ...
        self.__event = [
            x.value
            for x in list(self.__payload_type_query.find(self.event_context.message))
        ][0]
        self.__route_urls = [
            x.value
            for x in list(self.__remoteurl_query.find(self.event_context.message))
        ]
        self.__remote_url = self.__route_urls[0]
        self.__default_branches = [
            AbstractOrchestrator.normalize_branch_name(x.value)
            for x in list(
                self.__push_default_branch_query.find(self.event_context.message)
            )
        ]
        self.__repo_key = [
            x.value
            for x in list(
                self.__repo_project_key_query.find(self.event_context.message)
            )
        ][0]
        self.__repo_slug = [
            x.value
            for x in list(self.__repo_slug_query.find(self.event_context.message))
        ][0]
        self.__collection_url = [
            x.value
            for x in list(self.__collection_url_query.find(self.event_context.message))
        ][0]
    ...
    async def __common_execution_steps(self, services: CxOneFlowServices):
        ...
        repo_details = json_on_ok(
            await services.scm.exec(
                "GET",
                f"/{self.__collection}/{self.__repo_key}/_apis/git/repositories/{self.__repo_slug}",
            )
        )
    ...
    async def execute(self, services: CxOneFlowServices):
        if self.__event not in AzureDevOpsEnterpriseOrchestrator.__workflow_map.keys():
            AzureDevOpsEnterpriseOrchestrator.log().error(
                f"Unhandled scan event type: {self.__event}"
            )
        else:
            await self.__common_execution_steps(services)
            return await AzureDevOpsEnterpriseOrchestrator.__workflow_map[self.__event](
                self, services
            )
    ...
    async def handle_delegated_scan(self, services: CxOneFlowServices, scan_id: str):
        if (
            self.__event
            not in AzureDevOpsEnterpriseOrchestrator.__delegate_scan_handler_map.keys()
        ):
            AzureDevOpsEnterpriseOrchestrator.log().error(
                f"Unhandled delegated scan event type: {self.__event}"
            )
        else:
            self.delegated_scan = True
            await self.__common_execution_steps(services)
            return await AzureDevOpsEnterpriseOrchestrator.__delegate_scan_handler_map[
                self.__event
    ...
    async def is_signature_valid(self, shared_secret):
        ...
        if auth is None:
            AzureDevOpsEnterpriseOrchestrator.log().warning(
                "Authorization header is missing in request, rejecting."
            )
            return False
        ...
        if base64_payload is None:
            AzureDevOpsEnterpriseOrchestrator.log().warning(
                "Authorization header is not in the correct form, rejecting."
            )
            return False

        sent_secret = (
            base64.b64decode(base64_payload).decode("utf-8").split(":")[-1:].pop()
        )
    ...
    async def __get_protected_branches_from_policies(self, scm_service: SCMService):
        ...
        while True:
            resp = await scm_service.exec(
                "GET",
                f"/{self.__collection}/{self.__repo_key}/_apis/policy/configurations",
                query,
            )
            ...
            for p in policies:
                if (
                    "refName" in p.value.keys()
                    and p.value["repositoryId"] == self.__repo_id
                ):
                    branches.append(
                        AzureDevOpsEnterpriseOrchestrator.normalize_branch_name(
                            p.value["refName"]
                        )
                    )
        ...
        if len(prefix_branches) > 0:
            ...
            while True:
                resp = await scm_service.exec(
                    "GET",
                    f"/{self.__collection}/{self.__repo_key}/_apis/git/repositories/{self.__repo_id}/refs",
                    query,
                )
                ...
                ref_names = set(
                    [
                        ...
                        for ref in self.__branch_names_query.find(json_on_ok(resp))
                    ]
                )
    ...
    async def _get_protected_branches(self, scm_service: SCMService):
        return (
            self.__default_branches
            + await self.__get_protected_branches_from_policies(scm_service)
        )
    ...
    async def get_default_cxone_project_name(self) -> str:
        ...
        return AzureDevOpsProjectNaming.create_project_name(
            p.org, self._repo_project_key, self._repo_name
        )

    async def __is_pr_draft(self) -> bool:
        return bool(
            AzureDevOpsEnterpriseOrchestrator.__pr_draft_query.find(
                self.event_context.message
            )[0].value
        )

    def __populate_common_push_data(self):
        self.__source_branch = self.__target_branch = (
            AbstractOrchestrator.normalize_branch_name(
                [
                    x.value
                    for x in list(
                        self.__push_target_branch_query.find(self.event_context.message)
                    )
                ][0]
            )
        )
        self.__source_hash = self.__target_hash = [
            x.value
            for x in list(
                self.__push_target_hash_query.find(self.event_context.message)
            )
        ][0]
    ...
    async def _execute_push_scan_workflow(
        self, services: CxOneFlowServices, scan_tags: Dict[str, str] = None
    ):
        self.__populate_common_push_data()
        return await AbstractOrchestrator._execute_push_scan_workflow(
            self, services, scan_tags
        )

    def __populate_common_pr_data(self):
        self.__source_branch = AbstractOrchestrator.normalize_branch_name(
            [
                x.value
                for x in list(
                    self.__pr_frombranch_query.find(self.event_context.message)
                )
            ][0]
        )
        self.__target_branch = AbstractOrchestrator.normalize_branch_name(
            [
                x.value
                for x in list(self.__pr_tobranch_query.find(self.event_context.message))
            ][0]
        )
        self.__source_hash = [
            x.value
            for x in list(self.__pr_fromhash_query.find(self.event_context.message))
        ][0]
        self.__target_hash = [
            x.value
            for x in list(self.__pr_tohash_query.find(self.event_context.message))
        ][0]
        self.__pr_id = str(
            [
                x.value
                for x in list(self.__pr_id_query.find(self.event_context.message))
            ][0]
        )
        ...
        statuses = list(
            set(
                [
                    AzureDevOpsEnterpriseOrchestrator.__pr_status_map[x.value]
                    for x in AzureDevOpsEnterpriseOrchestrator.__pr_reviewer_status_query.find(
                        self.event_context.message
                    )
                ]
            )
        ...
        self.__pr_state = AzureDevOpsEnterpriseOrchestrator.__pr_state_query.find(
            self.event_context.message
        )[0].value
    ...
    async def _execute_pr_scan_workflow(
        ...
        if await self.__is_pr_draft():
            AzureDevOpsEnterpriseOrchestrator.log().info(
                f"Skipping draft PR {AzureDevOpsEnterpriseOrchestrator.__pr_self_link_query.find(self.event_context.message)[0].value}"
            )
            return
        ...
        if len(existing_scans) > 0:
            ...
            repo_details = await services.scm.exec(
                "GET",
                f"{self.__collection}/{self._repo_project_key}/_apis/git/repositories/{self.__repository_id}",
            )

            if not repo_details.ok:
                AzureDevOpsEnterpriseOrchestrator.log().error(
                    f"Response [{repo_details.status_code}] to request for repository details, event handling aborted."
                )
                return

            self.__default_branches = [
                AbstractOrchestrator.normalize_branch_name(
                    repo_details.json()["defaultBranch"]
                )
            ]

            return await AbstractOrchestrator._execute_pr_scan_workflow(
                self, services, scan_tags
            )
    ...
    def __repository_id(self) -> str:
        return [
            x.value
            for x in list(self.__repository_id_query.find(self.event_context.message))
        ][0]
