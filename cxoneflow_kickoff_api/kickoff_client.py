...
class KickoffClient:
    ...
    def __init__(
        self,
        private_ssh_key: str,
        private_key_password: Union[str, None],
        cxoneflow_ko_url: str,
        user_agent: str,
        ...
        self.__pkey = load_ssh_private_key(
            private_ssh_key.encode("UTF-8"), private_key_password
        )
    ...
    async def __get_jwt(self, force=False) -> str:
        async with self.__lock:
            if (
                force
                or self.__jwt is None
                or self.__jwt_exp is None
                or int(time.time()) >= self.__jwt_exp - KickoffClient.__JWT_JITTER
            ):
                self.log().debug("Renewing JWT")
                alg = get_signature_alg(self.__pkey)
                self.__jwt_exp = int(time.time()) + KickoffClient.__JWT_TIMEOUT

                payload = {"exp": self.__jwt_exp, "alg": alg}
    ...
    async def kickoff_scan(
        self,
        msg: KickoffMsg,
        waiting_callback: Callable[
            [KickoffStatusCodes, KickoffResponseMsg, int], bool
        ] = None,
        ...
        while True:
            ...
            if resp_status == KickoffStatusCodes.SCAN_STARTED:
                self.log().debug(
                    f"Scan started: {resp_msg.to_json()}"
                )  # pylint: disable=E1101
                return resp_status, resp_msg
            elif resp_status == KickoffStatusCodes.SCAN_EXISTS:
                self.log().debug(
                    f"The server indicated a kickoff scan is already running or has finished for {msg}"
                )
                return resp_status, resp_msg
            elif resp_status == KickoffStatusCodes.TOO_MANY_SCANS:

                if waiting_callback is not None and not waiting_callback(
                    resp_status, resp_msg, cur_sleep_delay
                ):
                    self.log().debug(
                        "The callback returned False to indicate the client should not retry submission."
                    )
                    return resp_status, resp_msg
                ...
                if retry_count % KickoffClient.__SLEEP_REPORT_MOD == 0:
                    if waiting_callback is None:
                        self.log().info(
                            f"The server indicated that too many concurrent scans are running."
                ...
                cur_sleep_delay = min(
                    KickoffClient.__SLEEP_MAX_SECONDS,
                    cur_sleep_delay + KickoffClient.__SLEEP_SECONDS,
                )
            else:
                raise KickoffClientException(
                    f"Response from {self.__service_url}: {resp.status_code} {resp.reason}"
                )
