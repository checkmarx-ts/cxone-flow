...
class ConfigurationException(Exception):
    ...
    def no_wildcard_routes(key_path):
        return ConfigurationException(
            f"A wildcard route that matches an arbitrary value is not valid for {key_path}"
        )
    ...
    def missing_keys(key_path, keys):
        return ConfigurationException(
            f"One or more of these elements are missing: {["/".join([key_path, x]) for x in keys]}"
        )

    @staticmethod
    def missing_at_least_one_key_path(key_path, keys):
        return ConfigurationException(
            f"At least one of these elements is required: {["/".join([key_path, x]) for x in keys]}"
        )
    ...
    def key_mismatch(key_path, provided, needed):
        return ConfigurationException(
            f"{key_path} invalid: Needed {needed} but provided {provided}."
        )

    @staticmethod
    def invalid_keys(key_path, keys: List):
        return ConfigurationException(
            f"These keys are invalid: {["/".join([key_path, x]) for x in keys]}"
        )

    @staticmethod
    def module_load_error(key_path, module_name):
        return ConfigurationException(
            f"Error loading module {module_name} at {key_path}."
        )

    @staticmethod
    def invalid_service_principal_config(key_path):
        return ConfigurationException(
            f"Service principal at {key_path} requires sp-client-secret."
        )

    @staticmethod
    def invalid_authorization_type(key_path):
        return ConfigurationException(
            f"The SCM authorization configured at {key_path} is not supported."
        )
...
class ConfigDictionaryReader:
    ...
    def _get_file_contents_from_value_of_key_or_default(config_dict, key, default):
        if not key in config_dict.keys():
            ...
            if not os.path.isfile(
                Path(CommonConfig._secret_root) / Path(config_dict[key])
            ):
                return default
            else:
                with open(
                    Path(CommonConfig._secret_root) / Path(config_dict[key]), "rt"
                ) as f:
                    return f.read().strip()

    @staticmethod
    def _get_secret_from_value_of_key_or_default(config_dict, key, default):
        return SecretRegistry.register(
            CommonConfig._get_file_contents_from_value_of_key_or_default(
    ...
    def _get_secret_from_value_of_key_or_fail(config_path, key, config_dict):
        retval = CommonConfig._get_secret_from_value_of_key_or_default(
    ...
    @staticmethod
    def _get_value_for_key_or_default_warn_deprecated(
        desired_key: str,
        deprecated_key: str,
        config_path: str,
        config_dict: Dict,
        ...
        if config_dict is None:
            ...
            CommonConfig.log().warning(
                f"{config_path}: {deprecated_key} is deprecated and may not function in the future.  Use {desired_key}."
            )
            key = deprecated_key
...
class CommonConfig(ConfigDictionaryReader):
    ...
    def get_default_ssl_verify_value():
        ...
        CommonConfig.log().warning(
            "A path to the default CA bundle could not be determined.  Please set the REQUESTS_CA_BUNDLE environment variable."
        )
        return False
    ...
    def _load_amqp_settings(config_path, **kwargs) -> Union[str, str, str, bool]:
        ...
        if not amqp_dict is None:
            amqp_url = CommonConfig._get_secret_from_value_of_key_or_default(
                amqp_dict,
                "amqp-url",
                CommonConfig._get_value_for_key_or_fail(
                    config_path, "amqp-url", amqp_dict
                ),
            )
            amqp_user = CommonConfig._get_secret_from_value_of_key_or_default(
                amqp_dict, "amqp-user", None
            )
            amqp_password = CommonConfig._get_secret_from_value_of_key_or_default(
                amqp_dict, "amqp-password", None
            )
            ssl_verify = CommonConfig._get_value_for_key_or_default(
                "ssl-verify", amqp_dict, CommonConfig.get_default_ssl_verify_value()
            )
    ...
    def _cxone_client_factory(config_path, **kwargs):
        ...
        tenant_name = CommonConfig._get_value_for_key_or_fail(
            config_path, "tenant", kwargs
        )

        iam_endpoint_value = CommonConfig._get_value_for_key_or_fail(
            config_path, "iam-endpoint", kwargs
        )
        ...
        api_endpoint_value = CommonConfig._get_value_for_key_or_fail(
            config_path, "api-endpoint", kwargs
        )
        ...
        if "api-key" in kwargs.keys():
            return cx.CxOneClient.create_with_api_key(
                CommonConfig._get_secret_from_value_of_key_or_fail(
                    config_path, "api-key", kwargs
                ),
                __agent__,
                tenant_auth_endpoint,
                tenant_api_endpoint,
                timeout=CommonConfig._get_value_for_key_or_default(
                    "timeout-seconds", kwargs, 60
                ),
                retries=CommonConfig._get_value_for_key_or_default(
                    "retries", kwargs, 3
                ),
                retry_delay_s=CommonConfig._get_value_for_key_or_default(
                    "retry-delay", kwargs, 30
                ),
                proxy=CommonConfig._get_value_for_key_or_default(
                    "proxies", kwargs, None
                ),
                ssl_verify=CommonConfig._get_value_for_key_or_default(
                    "ssl-verify", kwargs, CommonConfig.get_default_ssl_verify_value()
                ),
            )
        elif "oauth" in kwargs.keys():
            oauth_params = CommonConfig._get_value_for_key_or_fail(
                config_path, "oauth", kwargs
            )
            oauth_id = CommonConfig._get_secret_from_value_of_key_or_fail(
                f"{config_path}/oauth", "client-id", oauth_params
            )
            oauth_secret = CommonConfig._get_secret_from_value_of_key_or_fail(
                f"{config_path}/oauth", "client-secret", oauth_params
            )
