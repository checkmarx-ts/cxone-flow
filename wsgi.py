...
from orchestration.kickoff import (
    KickoffOrchestrator,
...
async def __kickoff_impl(orch: KickoffOrchestrator):
    ...
    try:
        if await OrchestrationDispatch.execute_kickoff(orch):
            ...
            msg = ko.KickoffResponseMsg(
                running_scans=orch.running_scans, started_scan=orch.started_scan
            )
        else:
    ...
    return Response(
        msg.to_json() if msg is not None else None,
        status=code.value,
...
async def bbc_webhook_endpoint():
    ...
    __log.debug(
        f"bbc webhook: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    try:
        TaskManager.in_background(
            OrchestrationDispatch.execute(
                BitBucketCloudOrchestrator(
                    EventContext(request.get_data(), dict(request.headers))
                )
            )
        )
...
async def bbc_kickoff_endpoint():
    ...
    __log.debug(
        f"bbc kickoff: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    ec = EventContext(request.get_data(), dict(request.headers))
    return await TaskManager.in_foreground(
        __kickoff_impl(
            BitBucketCloudKickoffOrchestrator(
                ko.BitbucketCloudKickoffMsg(**(ec.message)), ec
            )
        )
    )
...
async def bbdc_webhook_endpoint():
    ...
    __log.debug(
        f"bbdc webhook: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    try:
        TaskManager.in_background(
            OrchestrationDispatch.execute(
...
async def bbdc_kickoff_endpoint():
    ...
    __log.debug(
        f"bbdc kickoff: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    ec = EventContext(request.get_data(), dict(request.headers))
    return await TaskManager.in_foreground(
        __kickoff_impl(
            BitBucketDataCenterKickoffOrchestrator(
                ko.BitbucketDCKickoffMsg(**(ec.message)), ec
            )
        )
    )
...
async def github_webhook_endpoint():
    ...
    __log.debug(
        f"github webhook: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    try:
        orch = GithubOrchestrator(
            HeaderFilteredEventContext(
                request.get_data(), dict(request.headers), "User-Agent|X-(Git)?[H|h]ub"
            )
        )
...
async def github_kickoff_endpoint():
    ...
    __log.debug(
        f"github kickoff: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    ec = EventContext(request.get_data(), dict(request.headers))
    return await TaskManager.in_foreground(
        __kickoff_impl(
            GithubKickoffOrchestrator(ko.GithubKickoffMsg(**(ec.message)), ec)
        )
    )
...
async def adoe_webhook_endpoint():
    ...
    __log.debug(
        f"adoe webhook: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    try:
        orch = AzureDevOpsEnterpriseOrchestrator(
            EventContext(request.get_data(), dict(request.headers))
        )
...
async def adoe_kickoff_endpoint():
    ...
    __log.debug(
        f"adoe kickoff: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    ec = EventContext(request.get_data(), dict(request.headers))
    return await TaskManager.in_foreground(
        __kickoff_impl(
            AzureDevOpsKickoffOrchestrator(ko.AdoKickoffMsg(**(ec.message)), ec)
        )
    )
...
async def gitlab_webhook_endpoint():
    ...
    __log.debug(
        f"gitlab webhook: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    try:
        orch = GitlabOrchestrator(
            HeaderFilteredEventContext(
                request.get_data(),
                dict(request.headers),
                "User-Agent|X-Gitlab|Idempotency-Key",
            )
        )
...
async def gitlab_kickoff_endpoint():
    ...
    __log.debug(
        f"gitlab kickoff: headers: [{request.headers}] body: [{json.dumps(request.json)}]"
    )
    ec = EventContext(request.get_data(), dict(request.headers))
    return await TaskManager.in_foreground(
        __kickoff_impl(
            GitlabKickoffOrchestrator(ko.GitlabKickoffMsg(**(ec.message)), ec)
        )
    )
